...
- **Query-parameter keys**: camelCase (`?excludedStatuses=…&sortBy=newest&pageSize=20`), matching the typed search-param objects in app code. The URL is serialized app state, so keys mirror the code that reads them (TanStack Router / Google-style); forcing snake_case here only adds aliasing churn. The lowercase rule below governs paths/slugs/**values**, not query keys.
- **Query-parameter values**: stable lowercase identifiers, not display strings. Use a status `name` like `completed` / `in_progress`, never `Completed` or `In Progress`. For multiple values, repeat the key or comma-separate lowercase identifiers (`?excludedStatuses=completed,archived`).
