...
- Tenancy terminology: the tenant a user belongs to is a **"workspace"** in all USER-FACING copy (labels, headings, buttons, placeholders, toasts, nav, docs, marketing). It maps 1:1 to an Omni org (see [URL-GRAMMAR.md](./URL-GRAMMAR.md) Rule 9). Keep **"organization"** only in CODE and identity internals (the Better Auth `organization` plugin, `authClient.organization.*`, DB tables/columns, GraphQL fields, `*.organization.*` CloudEvent types, the `workspaceSlug` route param stays as-is). "Team" is reserved for org-internal sub-groups only, never the tenant itself. A creator's public identity is their `@handle` page, never called a "workspace". Do not rename code identifiers when changing user-facing copy. **This user/code split is intentional and permanent, not tech debt:** Better Auth owns the `organization` entity (its tables, endpoints, and types cannot be renamed without forking), so code stays "organization" indefinitely and a stable code term distinct from the display term is the intended end state. Do NOT plan a migration to normalize code identifiers (or the JWT `claims/organizations` wire contract) to "workspace" - it is invisible to users, breaks every consumer, and buys nothing
